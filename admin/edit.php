    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['user_id']) && !empty($_POST['user_id']) && isset($_POST['old_password']) && !empty($_POST['old_password']) && isset($_POST['new_password']) && !empty($_POST['new_password'])
                ...
                $userResult = mysqli_query($con, "Select * from `Users` WHERE (`id` = " . $_POST['user_id'] . " AND `password` = '" . $_POST['old_password']. "')");
                if ($userResult->num_rows > 0) {
                    $result = mysqli_query($con, "UPDATE `Users` SET `password` = '" . $_POST['new_password'] . "', `last_updated`=" . time() . " WHERE (`id` = " . $_POST['user_id'] . " AND `password` = '" . $_POST['old_password'] . "')");
                    if (!$result) {
